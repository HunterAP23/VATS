...
#     parser.add_argument("--third", type=float, widget="DecimalField", gooey_options={"min": 0, "max": 5.0})
#     parser.add_argument("--location", type=str, widget="FileChooser")
...
def main():
    settings_msg = "Subparser example demonstating bundled configurations " "for Siege, Curl, and FFMPEG"
    ...
    curl_parser.add_argument(
        "--connect-timeout", help="Maximum time in seconds that you allow curl's connection to take"
    )
    ...
    curl_parser.add_argument(
        "--dump-header", type=argparse.FileType(), help="Write the protocol headers to the specified file"
    )
    curl_parser.add_argument(
        "--progress-bar", action="store_true", help="Make curl display progress as a simple progress bar"
    )
    ...
    siege_parser.add_argument(
        "--delay", help="simulated user is delayed for a random number of seconds between one and NUM", type=int
    )
