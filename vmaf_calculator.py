...
class VMAF_Calculator:
    ...
    def __init__(self):
        ...
        self.config = Config_Handler(self.args).get_config_data()

        # if self.args.vmaf_version:
        #     self.vmaf_version = self.args.vmaf_version
        # else:
        #     self.vmaf_version = self.config_data["General"]["vmaf_version"]
        #
        # if self.args.psnr:
        #     self.psnr = self.args.psnr
        # else:
        #     self.psnr = self.config_data["Calculations"]["psnr"]
        #
        # if self.args.ssim:
        #     self.ssim = self.args.ssim
        # else:
        #     self.ssim = self.config_data["Calculations"]["ssim"]
        #
        # if self.args.ms_ssim:
        #     self.ms_ssim = self.args.ms_ssim
        # else:
        #     self.ms_ssim = self.config_data["Calculations"]["ms_ssim"]
        #
        # if self.args.model:
        #     self.model = self.args.model
        # else:
        #     self.model = self.config_data["Calculations"]["model"]
        #
        # if self.args.log:
        #     self.log = self.args.log
        # else:
        #     self.log = self.config_data["Calculations"]["log_format"]

        # for k, v in dict(vars(self.config_data)).items():
    ...
    def parse_arguments(self):
        ...
        threads_help = "Specify number of threads to be used for each instance (Default is 0 for \"autodetect\").\n"
        ...
        threads_help += "The recommended value of threads to use per instance is 4-6.\n\n"
        ...
        optional_args.add_argument("-i", "--instances", dest="instances", type=int, choices=[i for i in range(mp.cpu_count())], help=inst_help)

        rem_threads_help = "Specify whether or not to use remaining threads that don't make a complete instance to use for an instance (Default is off).\n"
        rem_threads_help += "For example, if your system has 16 threads, and you are running 5 instances with 3 threads each, then you will be using 4 * 3 threads, which is 12.\n"
        rem_threads_help += "This means you will have 1 thread that will remain unused.\n"
        rem_threads_help += "Using this option would run one more VMAF calculation instance with only the single remaining thread.\n"
        ...
        optional_args.add_argument("-r", "--rem-threads", dest="use_remaining_threads", action="store_true", help=rem_threads_help)
        ...
        optional_args.add_argument("-ms", "--ms-ssim", dest="ms_ssim", action="store_true", help=ms_ssim_help)
